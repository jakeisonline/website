...
<Callout type="note" title="A slight caveat">
  ...
  to wait. For example, a button that triggers an API call is an example of an
  asynchronous action.
</Callout>
...
## Show me where something is going to happen
...
The benefit of this level of pending state communication is twofold:

1. It moves the users attention to where the interface is going to update
2. It reduces the potential for jarring UI updates once the async action is finished
...
If you have a button that triggers a user action which requires that user to wait, you should communicate that no matter how quick you think that action is.

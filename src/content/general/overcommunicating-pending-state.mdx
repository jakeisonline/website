...
It might feel a bit overkill, but there's no harm in overcommunicating that the user's action has been registered, and something is happening. It sure beats just sitting there wondering.
...
If you have a button that triggers a user action which requires that user to wait, even for a couple hundred milliseconds, you should communicate that no matter how quick you think that action is.

...
Imagine a page with a large grid of images, or a page displaying lots of products to purchase, you don't want your user staring at blank spaces slowly being filled in as the images load. **Gross**. Instead, you can use a blur placeholder to give the user a quick visual indication that something is loading, without a jarring change.

<Example caption="Image loading has been simulated slightly slower loading times so you can see the effect. Thanks to Karsten Winegeart from Unsplash for the amazing pups.">
  <GalleryExample />
</Example>
...
<Example caption="Partly due to a lack of shame, we'll use my face for this example.">
  <SimpleExample />
</Example>

In order to create this blurred image, we'll need to perform two steps:

1. We should resize the original image to be much smaller than the original. Not only will this of course reduce the final size of the blurred image, it will keep the encoding performant. If you care about neither of these things, you can skip this step.
...
You could simply stretch the resized image to the desired size, use CSS to apply `blur` to it, and then wrap it in a container with the same size as the original with `overflow-hidden` to ensure the edges are crisp (CSS `blur` feathers anything it is applied to):
...
3. Now you've got to mess with markups _and_ CSS to get the desired effect.

...
<Callout type="tip">
  Just looking for the solution? Skip to the [Sticky detection with
...
### Quick `IntersectionObserver` overview
...
Sticky elements present us with a unique challenge when using `IntersectionObserver`: if the element is sticky, it unlikely intersects with the viewport until it hits the bounds of it's container element, at which point it becomes fixed in place 🤔.
...
By telling the observer to observe the element with a `rootMargin` of `-16px 0px 0px 0px`, for example, we're telling the observer to start observing the element when it's 16px from the top of the viewport.

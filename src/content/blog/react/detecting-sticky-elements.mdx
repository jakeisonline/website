...
**🧠 Here's the big brain bit:** by setting the `rootMargin` with a negative inset that matches the offset of the sticky element, we can reasonably deduce that when the element is within the `rootMagin` it is **not** sticky.

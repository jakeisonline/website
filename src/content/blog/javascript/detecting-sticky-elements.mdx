...
import StickyHeaderVideo from "@/assets/videos/luma-sitcky-date-headings.mp4"

I recently stumbled upon a lovely detail on [Luma's website](https://www.luma.com/) when scrolling down a list. As soon as the date headings became sticky, they turn into floating badges.

<Video src={StickyHeaderVideo} />

There's no way to use CSS to style an element when it becomes sticky, so how the hell did they do it?

<Callout type="note">
  Just looking for the solution? Sure, skip to [the final
  solution](#the-final-solution), but you'll miss my diagrams!
</Callout>
...
We'll also check if the element is intersecting where we expect it to be to avoid false positives (i.e. we think the element is sticking, but we just haven't scrolled past it yet).
...
stickyElements.forEach((stickyElement) => {
  ...
  const observer = new IntersectionObserver(
    ...
    {
      // 💫 Add a margin to the observer's root, to account for

...
export function ComponentCard({ component, children }: ComponentCardProps) {
  return (
    <NextLink href={`/components/${component}`} className="">
      <Card
        id={component}
        className="group hover:inner-border-link-hover/40 hover:shadow-link/30 w-full flex flex-col h-80 relative transition-all duration-300 border-0 inner-border-border inner-border-2"
      >
        {children}

export const COUNTRIES = [
  {
    country: "Albania",
    latitude: 41.1238814,
    longitude: 17.3856355,
    zoom: 7,
    infoText:
      "Sed porttitor blandit mi ac tincidunt. Maecenas diam est, sagittis eu dignissim nec, luctus vitae odio. Vestibulum et lobortis ante. Pellentesque vitae enim dictum, faucibus augue eu, aliquam metus.",
    infoBullets: [
      "Curabitur pretium at massa.",
      "Cras tempus diam at tellus mattis semper.",
      "Donec tristique urna at ex congue malesuada.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Czech Republic",
    latitude: 49.7736566,
    longitude: 12.830106,
    zoom: 7,
    infoText:
      "Duis quis lacus tristique, dictum turpis vel, eleifend risus. Fusce consequat vehicula arcu, in congue nulla feugiat ac. Nam vel quam vel purus lobortis posuere eget id metus. Sed.",
    infoBullets: [
      "Nulla vestibulum sem eu fermentum dignissim.",
      "Aenean luctus velit nec vulputate pharetra.",
      "Nam pretium orci in libero tempus egestas.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Germany",
    latitude: 51.0834196,
    longitude: 10.4234469,
    infoText:
      "Integer non elit turpis. Aenean auctor elit non dictum porttitor. Morbi rhoncus nisl non tortor maximus, id sagittis lacus placerat. Quisque massa magna, ornare vel consectetur sed, luctus nec.",
    infoBullets: [
      "Integer sodales felis eget pellentesque sollicitudin.",
      "In elementum sapien vel velit porttitor consequat.",
      "Nulla porta est et consectetur faucibus.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Greece",
    latitude: 38.9953683,
    longitude: 21.9877132,
    infoText:
      "Donec et blandit eros, non venenatis lorem. Curabitur mollis libero nec orci consectetur, vitae faucibus mauris viverra. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia.",
    infoBullets: [
      "Suspendisse eget nibh ut ligula maximus tempus.",
      "Cras egestas enim vel turpis condimentum, eu dignissim nibh aliquam.",
      "Sed quis mauris sed metus venenatis tempus a eu tellus.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Ireland",
    latitude: 53.3574484,
    longitude: -10.7438478,
    zoom: 5,
    infoText:
      "Fusce eget faucibus metus. Nam vel posuere dui. Aliquam erat volutpat. Curabitur luctus, nulla eget eleifend pretium, orci magna congue augue, et tincidunt ipsum augue a arcu. Sed molestie.",
    infoBullets: [
      "Suspendisse eget nibh ut ligula maximus tempus.",
      "Fusce vel orci rutrum, luctus neque facilisis, dignissim dui.",
      "Donec non nibh vehicula, tincidunt quam eget, porttitor nibh.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Italy",
    latitude: 42.6384261,
    longitude: 12.674297,
    infoText:
      "Fusce eget faucibus metus. Nam vel posuere dui. Aliquam erat volutpat. Curabitur luctus, nulla eget eleifend pretium, orci magna congue augue, et tincidunt ipsum augue a arcu. Sed molestie.",
    infoBullets: [
      "Donec sit amet nisl sodales, aliquam sem in, pulvinar mauris.",
      "Fusce vel orci rutrum, luctus neque facilisis, dignissim dui.",
      "Pellentesque varius felis nec diam malesuada luctus.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Kenya",
    latitude: 0.1649257,
    longitude: 32.59572,
    infoText:
      "Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae.. Sed molestie.",
    infoBullets: [
      "Cras non odio quis massa porttitor vulputate ac eget ex.",
      "Fusce vel orci rutrum, luctus neque facilisis, dignissim dui.",
      "Pellentesque varius felis nec diam malesuada luctus.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Poland",
    latitude: 52.215933,
    longitude: 19.134422,
    infoText:
      "Proin sodales blandit lorem, vel ullamcorper eros ornare id. Aliquam nibh massa, scelerisque non elit ac, mollis tincidunt mi. Nullam lacinia nunc nibh, ac imperdiet enim ultricies luctus. Aliquam.",
    infoBullets: [
      "Morbi molestie metus et lectus fermentum, a hendrerit magna ornare.",
      "Cras non odio quis massa porttitor vulputate ac eget ex.",
      "Integer mattis justo eu tempus viverra.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Portugal",
    latitude: 39.557191,
    longitude: -7.853535,
    infoText:
      "Praesent consequat, ipsum eu ultricies tincidunt, ante risus bibendum lectus, eget aliquam purus est eget orci. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae.",
    infoBullets: [
      "Mauris aliquam quam id elit pulvinar pharetra.",
      "Donec non nibh vehicula, tincidunt quam eget, porttitor nibh.",
      "Praesent sed quam volutpat, sodales massa ac, aliquam dolor.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "Spain",
    latitude: 39.3262345,
    longitude: -4.8380649,
    infoText:
      "Integer vulputate tincidunt accumsan. Morbi laoreet, quam et tempor mollis, justo sapien ultricies sapien, vel malesuada sem augue sed enim. Phasellus sit amet tincidunt tellus, eu bibendum lacus. Curabitur.",
    infoBullets: [
      "Suspendisse id ante ac nulla convallis dictum.",
      "Nunc elementum est in ante feugiat faucibus.",
      "Phasellus in ipsum id lectus tristique ultrices.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
  {
    country: "United Kingdom",
    latitude: 54.7023545,
    longitude: -3.2765753,
    infoText:
      "Morbi in tortor fermentum, eleifend elit at, egestas diam. Donec enim turpis, mollis a accumsan eget, porta ac ligula. Nunc lacinia id orci eget luctus. Sed vel augue rhoncus.",
    infoBullets: [
      "Sed quis mauris sed metus venenatis tempus a eu tellus.",
      "Donec sit amet nisl sodales, aliquam sem in, pulvinar mauris.",
      "Fusce vel orci rutrum, luctus neque facilisis, dignissim dui.",
    ],
    infoLabels: ["ipsum", "faucibus", "pellentesque", "lacus"],
  },
]
